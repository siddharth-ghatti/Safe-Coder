...
impl Worker {
    ...
    async fn execute_claude_code(&mut self) -> Result<String> {
        ...
        // Claude Code CLI usage: claude -p "prompt" --print --no-input
        // Or for file operations: claude "do something" in the working directory
        let mut cmd = Command::new(&self.cli_path);
        cmd.current_dir(&self.workspace)
            .arg("-p")
            .arg(&self.task.instructions)
            .arg("--print")  // Print output to stdout
            .arg("--no-input")  // Non-interactive mode
            .stdout(Stdio::piped())
            .stderr(Stdio::piped());
