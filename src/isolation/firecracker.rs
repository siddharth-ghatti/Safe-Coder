...
impl FirecrackerBackend {
    ...
    fn create_firecracker_config(&self, _shared_dir: &Path) -> Result<FirecrackerConfig> {
...
impl IsolationBackend for FirecrackerBackend {
    async fn start(&mut self, project_path: PathBuf) -> Result<PathBuf> {
        ...
        tracing::info!("🔥 Creating isolated copy of project in Firecracker VM");
        ...
        tracing::info!("🔥 Firecracker VM isolation active - agent confined to sandbox");
    ...
    async fn stop(&mut self) -> Result<()> {
        ...
        if let Some(instance) = self.instance.take() {
            ...
            tracing::info!("✓ Stopped Firecracker VM {}", instance.id);
        }
