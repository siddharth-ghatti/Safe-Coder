...
use safe_coder::tools::{AgentMode, ToolRegistry, ToolContext};
...
fn test_agent_mode_enabled_tools() {
    ...
    // Build mode should have all tools
...
mod mode_compatibility_tests {
    ...
    fn test_mode_tool_compatibility() {
        let dangerous_tools = ["write_file", "edit_file", "bash", "subagent"];
        ...
        for tool in dangerous_tools.iter() {
            assert!(!plan_mode.is_tool_enabled(tool), 
                "Plan mode should not enable dangerous tool: {}", tool);
        }
        
        // Plan mode should allow safe tools
        for tool in safe_tools.iter() {
            assert!(plan_mode.is_tool_enabled(tool), 
                "Plan mode should enable safe tool: {}", tool);
        }
        
        // Build mode should allow all tools
        for tool in dangerous_tools.iter() {
            assert!(build_mode.is_tool_enabled(tool), 
                "Build mode should enable tool: {}", tool);
        }
